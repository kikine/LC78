...
$ftpUse		= mosGetParam( $_REQUEST, "ftpUse", "0" );
$ftpUserName	= mosGetParam( $_REQUEST, "ftpUserName", "" );
$ftpPassword	= mosGetParam( $_REQUEST, "ftpPassword", "" );
$ftpHostName	= mosGetParam( $_REQUEST, "ftpHostName", "" );
$path = $mosConfig_absolute_path . "/administrator/components/com_installer/$element/$element.php";

$ftpDB = new ftpHostAccnt;
	$ftpDB->ftpUserName = $ftpUserName;
	$ftpDB->ftpPassword = '';
	$ftpDB->ftpHostName = $ftpHostName;
	$ftpDB->enable = $ftpUse ? 1 : 0;
	$ftpDB->setFtpAccnt();
...
function uploadPackage( $installerClass, $option, $element, $client ) {
	global $mainframe, $ftp, $ftpUse, $ftpUserName, $ftpPassword, $ftpHostName;
	
	$installer = new $installerClass();
	$installer->setFtpUse($ftpUse);
	$installer->setFtpUserName($ftpUserName);
	$installer->setFtpPassword($ftpPassword);
	$installer->setFtpHostName($ftpHostName);
	$installer->setFtpGlobals();
	
	// Note that the FTP connection needs to be established
		// before a zip extraction!
		if($ftpUse)establishFtpConn($installer);
	...
	if ($resultdir !== false) {
		...
		HTML_installer::showInstallMessage( $installer->getError(), 'Upload '.$element.' - '.($ret ? 'Success' : 'Failed'),
			$installer->returnTo( $option, $element, $client ) );
		$installer->cleanupInstall( $userfile['name'], $installer->unpackDir());
	} else {
		HTML_installer::showInstallMessage( $msg, 'Upload '.$element.' -  Upload Error',
			$installer->returnTo( $option, $element, $client ) );
	}
	if($ftpUse) $ftp->ftpClose();
}
