...
class mosInstallerComponent extends mosInstaller {
	...
	*/
    ...
    if (file_exists($this->elementDir())) {
      $this->setError( 1, 'Another component is already using directory: "' . $this->elementDir() . '"' );
		...
		if(!file_exists($this->elementDir()) && !$this->makedirectory($this->elementDir(),0777)) {
			$this->setError( 1, 'Failed to create directory "' . $this->elementDir() . '"' );
			return false;
		}

		if(!file_exists($this->componentAdminDir()) && !$this->makedirectory($this->componentAdminDir(),0777)) {
	...
	*/
	function uninstall( $cid, $option, $client=0 ) {
		...
		$row->admindir = $mosConfig_absolute_path."/administrator/components/".$row->option;
		$row->comdir = $mosConfig_absolute_path."/components/".$row->option;
		$compDirs = array($row->admindir, $row->comdir);
		...
		if(!$this->getFtpUse()){
//		if($perms['adminCompsDir'] != '777'){
		if(!is_deletable($mosConfig_absolute_path.'/administrator/components')){
			HTML_installer::showInstallMessage("The Mambo administrator/components/ subdirectory permissions of ".$perms['adminCompsDir']." will prevent an Uninstall.  'FTP Assist' option MUST be used for Uninstall or directory permissions need to be manually changed thru a FTP client", 'Uninstall -  error',
			 $this->returnTo( $option, 'component', $client ));
			exit();
		}
	
//		if($perms['compsDir'] != '777'){
		if(!is_deletable($mosConfig_absolute_path.'/components')){
			HTML_installer::showInstallMessage("The Mambo components/ subdirectory permissions of ".$perms['compsDir']." will prevent an Uninstall.  'FTP Assist' option MUST be used for Uninstall or directory permissions need to be manually changed thru a FTP client", 'Uninstall -  error', 
			$this->returnTo( $option, 'component', $client ));
			exit();
		}
	
	
//		if(!$owner[$row->admindir] && $perms[$row->admindir] != '777'){
		if(!is_deletable($row->admindir)){
			// error, can't uninstall this component without FTP assist
			HTML_installer::showInstallMessage("Directory $row->admindir for Component $row->name is not removable unless the 'FTP Assist' option is used for Uninstall or directory permissions are manually changed thru a FTP client", 'Uninstall -  error',
			 $this->returnTo( $option, 'component', $client ));
			exit();
		}

//		if(!$owner[$row->comdir] && $perms[$row->comdir] != '777'){
		if(!is_deletable($row->comdir)){
			// error, can't uninstall this component without FTP assist
			HTML_installer::showInstallMessage("Directory $row->comdir for Component $row->name is not removable unless the 'FTP Assist' option is used for Uninstall or directory permissions are manually changed thru a FTP client", 'Uninstall -  error',
			 $this->returnTo( $option, 'component', $client ));
			exit();
		}

		// Now go check the component's actual subdirectories if any to make sure we can delete them ok too.
		if(needFtpForCompDirs($compDirs)){
			HTML_installer::showInstallMessage("One or more subdirectories within the Component's $row->admindir or $row->comdir directories are not removable unless the 'FTP Assist' option is used for Uninstall or directory permissions are manually changed thru a FTP client", 'Uninstall -  error',
			 $this->returnTo( $option, 'component', $client ));
			exit();
		}
	}
