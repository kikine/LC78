...
class HTML_easygb {
  ...
  function showGBEntries($rows, $pageNav){
    ...
    <div class="componentheading">Livre D'Or </div>
	  <table class="adminform" width="100%" cellpadding="0" cellspacing="0">
	  <tr>
	    <td>Messages <?php echo ($pageNav->limitstart > 0 ? $pageNav->limitstart : 1); ?> - <?php echo ($pageNav->limitstart + $pageNav->limit < $pageNav->total ? $pageNav->limitstart + $pageNav->limit : $pageNav->total); ?> sur <?php echo $pageNav->total; ?> Messages </td>
	    <td><div style="float: right"><a href="index.php?option=<?php echo $option; ?>&task=new&Itemid=<?php echo $Itemid; ?>">Ecrire un message</a></div></td>
	  </tr>
	  <tr>
  ...
  function editentry($row){
    ...
    ?>
    <form action="index2.php" method="post" name="adminForm">
    <div class="componentheading">Postez un nouveau message dans le Livre d'or </div>
    <table class="adminform" width="100%" cellpadding="0" cellspacing="0">
	  <tr>
	  ...
	  <tr valign="top">
	    ...
	    <td>
	      ...
	      Entez le code visible sur l'image ci-dessus:<br />
	      <input type="text" name="captcha_code" size="10" value="" />
	    </td>
	  </tr>
	  <tr>
	    <td>&nbsp;</td>
	    <td><input type="button" class="button" onclick="submitbutton('save');" value="Envoyer !" /></td>
	  </tr>
	  </table>
