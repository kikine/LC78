...
|                                                                     |
-----------------------------------------------------------------------
* @version $Id: view.php,v 1.70 2006/11/28 00:24:23 kevinuru Exp $
* @package zOOmGallery
...
if ($zoom->_gallery->_images[$key]->isMember($popup)) {
	...
	//$fullsize = $mosConfig_live_site."/".$zoom->_CONFIG['imagepath'].$zoom->_gallery->_dir."/".$zoom->_gallery->_images[$key]->_filename;
	...
	if ($zoom->isImage($zoom->_gallery->_images[$key]->_type)) {
		...
		ZMG_Template_View::showTypeDocument($zoom->_gallery->_images[$key]->_thumbnail, $zoom->_CONFIG['popUpImages']);
