...
|                                                                     |
-----------------------------------------------------------------------
* @version $Id: ajaxcallback.php,v 1.45 2006/11/27 00:58:06 kevinuru Exp $
* @package zOOmGallery
...
if (md5($zoom->_CONFIG['secret']) == $id) {
	...
	switch ($task) {
	   ...
	   case 'catsmgr_getlist' :
	       ...
	       echo ("<tree id=\"0\">"
	        //. "<item text=\""._ZOOM_HD_NEW."\" id=\"new\" im0=\"page.gif\" im1=\"page.gif\" im2=\"page.gif\"></item>"
	   ...
	   case 'catsmgr_new' :
           ...
           $name       = urldecode($zoom->getParam($_REQUEST, 'name'));
           $dir        = urldecode($zoom->getParam($_REQUEST, 'dir'));
           $catpass    = urldecode($zoom->getParam($_REQUEST, 'password'));
           $keywords   = urldecode($zoom->getParam($_REQUEST, 'keywords'));
           $descr      = urldecode($zoom->getParam($_REQUEST, 'descr', null, _MOS_ALLOWHTML));
           ...
           if (!empty($name)){
            	...
            	if ($zoom->createdir($zoom->_CONFIG['imagepath'].$mkdir, '777')){
            		...
            		$database->setQuery("INSERT INTO #__zoom (catname, catdescr, catdir, catpassword, catkeywords, "
            		 ...
            		 . " VALUES "
            		 . "('".$zoom->escapeString($zoom->htmlnumericentities($name))."', '".$zoom->escapeString($descr)."', '".$zoom->escapeString($mkdir)."', '"
            		 . $zoom->escapeString($pass)."', '".$zoom->escapeString($zoom->htmlnumericentities($keywords))."', '$parent', '$pos', '$hidemsg', "
	   ...
	   case 'catsmgr_save' :
	       ...
	       $parent    = intval($zoom->getParam($_REQUEST, 'parent'));
           $newname   = urldecode($zoom->getParam($_REQUEST, 'name'));
           $catpass   = urldecode($zoom->getParam($_REQUEST, 'password'));
           $keywords  = urldecode($zoom->getParam($_REQUEST, 'keywords'));
           $newdescr  = urldecode($zoom->getParam($_REQUEST, 'descr', null, _MOS_ALLOWHTML));
           $hidemsg   = intval($zoom->getParam($_REQUEST, 'hide_msg'));
           $shared    = intval($zoom->getParam($_REQUEST, 'shared'));
           $published = intval($zoom->getParam($_REQUEST, 'published'));
           $selections = $zoom->getParam($_REQUEST, 'members_sel');
           if (isset($newname)) {
               ...
               $database->setQuery("UPDATE #__zoom "
                . "SET catname='".$zoom->escapeString($zoom->htmlnumericentities($newname))."', catdescr='".$zoom->escapeString($newdescr)."', "
                . " catpassword='".$zoom->escapeString($catpass)."', catkeywords='".$zoom->escapeString($zoom->htmlnumericentities($keywords))."', "
