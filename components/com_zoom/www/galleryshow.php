...
|                                                                     |
-----------------------------------------------------------------------
* @version $Id: galleryshow.php,v 1.53 2006/12/09 07:32:57 kevinuru Exp $
* @package zOOmGallery
...
$mainframe->addCustomHeadTag('<script language="javascript" type="text/javascript" src="'.$mosConfig_live_site.'/components/com_zoom/lib/js/scriptaculous.js?load=mm"></script>');
...
if (!$catid) {
	...
	if ($valid && $zoom->_gallery->isMember()) {
		...
		$mainframe->setPageTitle($zoom->_gallery->_name);
		...
		if ($ViewRecordCount % $zoom->_CONFIG['PageSize'] == 0) {
			$MaxPage = ceil($ViewRecordCount / $zoom->_CONFIG['PageSize']);
		} else {
			$MaxPage = ceil($ViewRecordCount / $zoom->_CONFIG['PageSize']);
		}
			...
			if ($zoom->_CONFIG['lightbox'] && $zoom->_gallery->getNumOfImages() > 0) {
				...
				<td align="right" class="sectiontableheader">
					...
					<a href="javascript:void(0);" onmouseover="return overlib('<?php echo _ZOOM_LIGHTBOX_GALLERY;?>', CAPTION, '<?php echo _ZOOM_LIGHTBOX;?>');" onmouseout="return nd();">
					   <img src="components/com_zoom/www/images/lightbox.png" border="0" name="lightbox" onclick="Zoom.lightboxAdd(<?php echo $zoom->_gallery->_id; ?>, 2, this);" alt="" />
				    </a>
					</div>
				</td>
				<?php
		...
		<?php
		//if (count($zoom->_gallery->_subcats) > 0) {
			?>
		<div align="center">
    		<table border="0" cellspacing="0" cellpadding="3" width="100%">
    			<tr>
