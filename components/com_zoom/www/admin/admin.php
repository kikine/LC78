...
|                                                                     |
-----------------------------------------------------------------------
* @version $Id: admin.php,v 1.16 2006/11/27 00:34:20 kevinuru Exp $
* @package zOOmGallery
...
if (isset($action)){
	...
	if($zoom->_isBackend) {
		...
		mosRedirect($mosConfig_live_site."/index.php?option=com_zoom&Itemid=".$Itemid."&page=admin", _ZOOM_OPTIMIZE_SUCCESS);	
	}
}
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="5">
                	<tr>
						<td width="100%" valign="top">
							...
							if ($zoom->_isAdmin || ($zoom->privileges->hasPrivilege('priv_creategal') | $zoom->privileges->hasPrivilege('priv_editgal') | $zoom->privileges->hasPrivilege('priv_delgal'))) {
								if ($zoom->_isBackend) {
									$link = "index".$backend.".php?option=com_zoom&amp;Itemid=".$Itemid."&amp;page=catsmgr";
								}
								else {
									$link = sefReltoAbs("index.php?option=com_zoom&amp;Itemid=".$Itemid."&amp;page=catsmgr");
								}
                			...
                			if ($zoom->_isAdmin || ($zoom->privileges->hasPrivilege('priv_upload') | $zoom->privileges->hasPrivilege('priv_editmedium') | $zoom->privileges->hasPrivilege('priv_delmedium'))) {
								if ($zoom->_isBackend) {
									$link = "index".$backend.".php?option=com_zoom&amp;Itemid=".$Itemid."&amp;page=mediamgr";
								}
								else {
									$link = sefReltoAbs("index.php?option=com_zoom&amp;Itemid=".$Itemid."&amp;page=mediamgr");
								}
								quickiconButton( $link, 'mediamgr_f2.png', _ZOOM_MEDIAMGR, '/components/com_zoom/www/images/admin/', _ZOOM_MEDIAMGR_DESCR );
               				}
							if ($zoom->_isBackend) {
								$link = "index".$backend.".php?option=com_zoom&amp;Itemid=".$Itemid."&amp;page=zoomthumb";
							}
							else {
			                ...
			                if ($zoom->_isAdmin) {
			                	if ($zoom->_isBackend) {	
									$link = "index".$backend.".php?option=com_zoom&amp;Itemid=".$Itemid."&amp;page=settings";
								}
								else {
									$link = sefReltoAbs("index.php?option=com_zoom&amp;Itemid=".$Itemid."&amp;page=settings");
								}
								quickiconButton( $link, 'settings_f2.png', _ZOOM_SETTINGS, '/components/com_zoom/www/images/admin/', _ZOOM_SETTINGS_DESCR );
								if ($zoom->_isBackend) {
									$link = "index".$backend.".php?option=com_zoom&amp;Itemid=".$Itemid."&amp;page=admin&amp;action=optimize";
								} else {
									$link = sefReltoAbs("index.php?option=com_zoom&amp;Itemid=".$Itemid."&amp;page=admin&amp;action=optimize");	
								}
	                ...
	                <tr>
						<td align="left" style="text-align:left;">
                        	<a href="<?php echo ($zoom->_isBackend) ? "index2.php?option=com_zoom&amp;Itemid=".$Itemid."&amp;page=credits" : sefReltoAbs("index.php?option=com_zoom&amp;Itemid=".$Itemid."&amp;page=credits");?>">
