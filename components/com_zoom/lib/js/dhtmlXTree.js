...
*   @topic: 0  
*/	
function dhtmlXTreeItemObject(itemId,itemText,parentObject,treeObject,actionHandler,mode,hoverHandler,evData){
	this.htmlNode="";
	this.acolor=""; 	
	this.scolor="";
	this.tr=0;
...
*     @topic: 2  
*/
	dhtmlXTreeObject.prototype._attachChildNode=function(parentObject,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,beforeNode,evData){
			...
			var itemHoverHandler = (this.hFunc && typeof(this.hFunc) == "object") ? this.hFunc : null;
			...
			Nodes[Count]=new dhtmlXTreeItemObject(itemId,itemText,parentObject,this,itemActionHandler,1,itemHoverHandler,evData);
...
*     @topic: 2
*/
	dhtmlXTreeObject.prototype.insertNewChild=function(parentId,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,evData){
		return this.insertNewItem(parentId,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,evData);
	}
...
*     @topic: 2
*/
	dhtmlXTreeObject.prototype.insertNewItem=function(parentId,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,evData){
		var parentObject=this._globalIdStorageFind(parentId);
		if (!parentObject) return (-1);
		return this._attachChildNode(parentObject,itemId,itemText,itemActionHandler,image1,image2,image3,optionStr,childs,null,evData);
	};
/**  
*     @desc: create new node as a child to specified with parentId
*     @type: public
	...
	//alert("x"+dhtmlObject.parsCount+"-"+parentId);
		...
		for(var i=0; i<node.childNodes.length; i++)
		{
	  		if ((node.childNodes[i].nodeType==1)&&(node.childNodes[i].tagName == "item"))
			{
				...
				var evData = {
				    published: nodx.getAttribute("published"),
				    shared: nodx.getAttribute("shared"),
				    keywords: nodx.getAttribute("keywords"),
				    uid: nodx.getAttribute("uid"),
				    subcat_id: nodx.getAttribute("subcat_id")
				}
						...
						temp.XMLload=1;
						var newNode=dhtmlObject.insertNewItem(parentId,cId,name,0,im0,im1,im2,zST,chd,evData);
...
//        htmlObject.childNodes[0].className="selectedTreeRow";
    ...
    dhtmlXTreeObject.prototype.onRowHover=function(e,htmlObject){
        if (!htmlObject) htmlObject=this.parentObject.span.parentNode;
	    if (window.event) e=event;
	    if (e && htmlObject.parentObject.treeNod.hFunc)
	       htmlObject.parentObject.treeNod.hFunc[0](htmlObject.parentObject);
	}
	
	dhtmlXTreeObject.prototype.onRowOut=function(e,htmlObject){
        if (!htmlObject) htmlObject=this.parentObject.span.parentNode;
	    if (window.event) e=event;
	    if (e && htmlObject.parentObject.treeNod.hFunc)
	       htmlObject.parentObject.treeNod.hFunc[1](htmlObject.parentObject);
	}
...
//		itemObject.span.appendChild(document.createTextNode(itemObject.label));
		...
		if (this.hFunc) {
		    td1.onmouseover = this.onRowHover; td1.onmouseout  = this.onRowOut;
		    td2.onmouseover = this.onRowHover; td2.onmouseout  = this.onRowOut;
		}
...
*/
	...
	dhtmlXTreeObject.prototype.setOnHoverHandler=function(func){  if (typeof(func)=="object") this.hFunc=func; else this.hFunc=eval(func);  };
...
*/	
	dhtmlXTreeObject.prototype._moveNode=function(itemObject,targetObject){
		...
		if (targetObject.childsCount && !targetObject.childNodes[0].childsCount)
		  targetObject = targetObject.childNodes[0];
		if (mode==1)
        {
            var z=targetObject;
            ...
            var nodeLevelA = this._getNodeLevel(nodeA,0);
            var nodeLevelB = this._getNodeLevel(nodeB,0)
            if (nodeLevelA>nodeLevelB)
                {
                return this._moveNodeTo(itemObject,nodeA.parentObject);
                }
            else
                {
                //11-23-2006: ZMG hack applied to add sorting by label functionality (mike).
                z=nodeB;
                if (nodeLevelA<nodeLevelB) {
                    while (z){
        				if (z.label.toLowerCase() < itemObject.label.toLowerCase())
        				    z=this._getNextNode(z);
        				if ((z==-1)) { z=false; }
                    }
                }
//  				if (this._checkParenNodes(itemObject.id,d)) return;
                return this._moveNodeTo(itemObject,nodeB.parentObject,z);
                }
...
//			alert(Nodes[targetObject.childsCount-2].id)
		...
		if (this.dropFunc) this.dropFunc(itemObject.id,targetObject.id,(beforeNode?beforeNode.id:null),itemObject.treeNod,targetObject.treeNod,this.dragactiveobj);
...
*/
	...
	dhtmlXTreeObject.prototype.setOnDropHandler=function(func){  if (typeof(func)=="function") this.dropFunc=func; else this.dropFunc=eval(func);  };
...
dhtmlXTreeObject.prototype._drag=function(sourceHtmlObject,dhtmlObject,targetHtmlObject){
		...
		//if (z.lastSelected)   z._setMove(z.lastSelected);
		 if ((!this.dragMove)||(this.dragMove()))
		 	{
		 	  this.dragactiveobj=targetHtmlObject;
