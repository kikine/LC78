...
timedSlideShow.prototype = {
	initialize: function(element, data) {
		...
		for(i=0;i<data.length;i++)
		{
			...
			currentImg.currentOpacity = new Effect.Opacity(currentImg, {duration: 400});
	...
	startSlideShow: function() {
		...
		Element.setOpacity(this.slideElements[parseInt(this.currentIter)], 1);
	...
	doSlideShow: function() {
		if (this.slideShowInit == 1)
		{
			...
			if (this.currentIter != 0) {
				new Effect.Opacity(this.slideElements[parseInt(this.currentIter)], {
				    from: 0.0,
				    to: 1.0,
				    duration: 0.5,
				    afterFinish = function() {
    					Element.setOpacity(this.slideElements[parseInt(this.lastIter)], 0);
    				}.bind(this),
				});
			} else {
				Element.setOpacity(this.slideElements[parseInt(this.currentIter)], 1);
				new Effect.Opacity(this.slideElements[parseInt(this.lastIter)], {
				    from: 1.0,
				    to: 0.0,
				    duration: 0.5});
			}
	...
	showInfoSlideShow: function() {
		...
		Element.setOpacity(this.slideInfoZone,0);
		...
		this.slideInfoZone.className = 'slideInfoZone';
		new Effect.Parallel([
    		new Effect.Opacity(this.slideInfoZone, {
    		    from: 0,
    		    to: 0.7}),
    		new Effect.SlideUp(this.slideInfoZone, {
    		})],
        	{
    		    duration 0.5
    		});
